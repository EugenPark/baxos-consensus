...
func (rp *Replica) randomBackOff(instance int) {
	...
	if baxosInstance.decided {
		rp.debug(fmt.Sprintf("PROPOSER: Instance %d: already decided, hence ignoring the timeout indication", instance), 3)
		return
	}
...
func (rp *Replica) tryPrepare() {
	if rp.baxosConsensus.isPreparing || rp.baxosConsensus.isBackingOff || len(rp.incomingWriteRequests) == 0 {
		rp.debug("PROPOSER: Already proposing or backing off, hence ignoring the propose request", 3)
