...
CMD [ "sh", "-c", "/baxos/bin/baxos-replica -id=\"${REPLICA_ID}\" -region=${REPLICA_REGION} -logFilePath=/logs/ -config=\"./configuration/values.yaml\" -debugLevel=100 -roundTripTime=40000 -artificialLatency=20000 -artificialLatencyMultiplier=10" ]
